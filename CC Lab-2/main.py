...
def register(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    # Check for existing username first to avoid relying on exceptions
    existing = db.execute("SELECT 1 FROM users WHERE username=?", (username,)).fetchone()
    if existing:
        return templates.TemplateResponse(
            "register.html",
            {"request": request, "error": "❌ Username already exists. Try a different one."},
            status_code=400,
        )
...
def events(request: Request, user: str):
    db = get_db()
    # Select only the needed columns and let the DB do work
...
def register_event(event_id: int, user: str):
    ...
    # Verify the event exists before registering to avoid unexpected crashes
...
def my_events(request: Request, user: str):
    ...
    # No expensive CPU-bound work here; let the DB do the heavy lifting above
